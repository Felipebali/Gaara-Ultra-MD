...
const handler = async (m, { conn, command, text }) => {
  ...
  if (command === 'banuser') {
    ...
    const userName = await conn.getName(userJid) || 'Usuario';
    if (!db[userJid]?.banned) return await conn.sendMessage(m.chat, { text: `${emoji} *${userName}* no está baneado.`, mentions: [userJid] });
    db[userJid].banned = false;
    db[userJid].banReason = '';
    db[userJid].bannedBy = null;
    await conn.sendMessage(m.chat, { text: `${done} *${userName}* ha sido desbaneado.`, mentions: [userJid] });
    ...
    if (db[userJid]?.banned) {
      ...
      await conn.sendMessage(m.chat, { text: `${emoji} *${userName}* está baneado.\nBaneado por: ${bannedBy}\nMotivo: ${reason}`, mentions: [userJid] });
    } else {
      await conn.sendMessage(m.chat, { text: `${done} *${userName}* no está baneado.`, mentions: [userJid] });
    }
    ...
    for (const [jid, data] of bannedEntries) {
      const userName = await conn.getName(jid) || 'Usuario';
...
handler.before = async function (m, { conn }) {
  ...
  if (db[sender]?.banned) {
    try {
      ...
      console.log(`[AUTO-KICK] Eliminado baneado ${sender} del grupo ${m.chat} por: ${db[sender].banReason || 'No especificado'}`);
    } catch (e) {
...
handler.participantsUpdate = async function (event) {
  ...
  if (action === 'add' || action === 'invite') {
    for (const user of participants) {
      ...
      const userName = await conn.getName(normalizedUser) || 'Usuario';
      ...
      if (db[normalizedUser].banned) {
        try {
          ...
          console.log(`[AUTO-KICK JOIN] ${normalizedUser} eliminado del grupo ${id} por: ${db[normalizedUser].banReason || 'No especificado'}`);
        } catch (e) {
